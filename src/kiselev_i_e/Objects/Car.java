...
public class Car {
    ...
    public void draw(Graphics2D g2d, int screenWidth, int screenHeight, double curvature) {

        int carX = screenWidth / 2;
        int carY = screenHeight - 95;
        int scale = 4;


        drawHeadlights(g2d, carX, carY, scale);


        drawCarBody(g2d, carX, carY, scale);
    }

    private void drawHeadlights(Graphics2D g2d, int carX, int carY, int scale) {
    ...
    private void drawCarBody(Graphics2D g2d, int carX, int carY, int scale) {
        ...
        g2d.fillRect(-2 * scale, 20 * scale, 4 * scale, 3 * scale);
